...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`Cron job: 找到 ${unpaidOrders.length} 个未付款订单需要发送提醒`)
    ...
    for (const order of unpaidOrders) {
      try {
        ...
        if (existingReminder) {
          console.log(`Cron job: 订单 ${order.id} 已发送过提醒邮件，跳过`)
          continue
        }
        ...
        console.log(`Cron job: 已发送付款提醒邮件给订单 ${order.id}`)
      } catch (error) {
        console.error(`Cron job: 发送付款提醒邮件失败，订单 ${order.id}:`, error)
        results.push({
          orderId: order.id,
          success: false,
    ...
    console.error('Cron job: 处理付款提醒失败:', error)
    return NextResponse.json({ 
      error: '处理付款提醒失败',
      timestamp: new Date().toISOString()
    }, { status: 500 })
