...
import { getPopularPackages } from '@/lib/actions/packages';

export const FeaturedPackages: React.FC = async () => {
  const packages = await getPopularPackages(8);

  const handleViewClick = (packageId: string) => {
    return `/packages/${packageId}`;
  };

  const handleBuyClick = (packageId: string) => {
    return `/packages/${packageId}/order`;
  };
            ...
            {packages.length > 0 ? (
              <>
                {packages.map((package_) => (
                  <div key={package_.id} className="flex-shrink-0 w-[300px] md:w-auto mx-2 first:ml-0 last:mr-0 md:mx-0">
                    <PackageCard
                      {...package_}
                      onViewClick={() => window.location.href = handleViewClick(package_.id)}
                      onBuyClick={() => window.location.href = handleBuyClick(package_.id)}
                    />
                  </div>
                ))}
                ...
                <Card className="overflow-hidden flex-shrink-0 w-[300px] md:w-auto mx-2 first:ml-0 last:mr-0 md:mx-0 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-dashed border-blue-200 hover:border-blue-300 hover:shadow-lg transition-all duration-300 group cursor-pointer">
                  <a href="/packages" className="p-8 flex flex-col items-center justify-center w-full h-full min-h-[400px] text-center">
                    ...
                    <p className="text-gray-600 text-sm">发现更多适合您的服务套餐</p>
